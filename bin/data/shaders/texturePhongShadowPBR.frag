...
void main() 
{
	vec3 texcol = texture(tex, texcoord_varying).rgb;
	float paint = texcol.r;
	texcol = pow(texcol, vec3(2.2));

	vec3 albedo = pow(texture(albedoMap, texcoord_varying).rgb, vec3(2.2));
	albedo = mix(texcol, albedo, paint);
